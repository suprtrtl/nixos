...
case "$(printf "  Applications\n  Tools\n󱄅  Nix\n  Learn\n  Power" | tofi --width=20% )" in
	...
	"  Learn")
		case "$(printf "󰈙  man\n  TLDR\n  Cheat\n󰖟  cheat.sh (Online)" | tofi)" in
			...
			"󰖟  cheat.sh (Online)") ghostty -e sh -c "curl cheat.sh/"$(curl cheat.sh/:list | tofi --width=20% --placeholder-text="search cheat.sh or enter known menu:")" && read -n 1 -s -r -p "Press any key to continue..."" ;;
